...
class HeartZoneStateTests: XCTestCase {
    ...
    override func setUp() {
        zoneStateManagerMock = ZoneStateManagerMock()
    ...
    func testHeartZoneStateWithoutActiveZoneSettings() {
        sut = HeartZoneNotAvailableState(
            stateManager: zoneStateManagerMock, settingsService: settingsServiceFake
        )
    ...
    func testHeartZoneActiveStateChangeZone() {
        ...
        sut = HeartZoneActiveState(
            zone: zone1, stateManager: zoneStateManagerMock, movement: .up,
    ...
    func testHeartZoneActiveStateFromTargetZoneWhenAlertEnabled() {
        ...
        let (zone1, _, _) = getNeighbourZones()
        sut = HeartZoneActiveState(
            zone: targetZone, stateManager: zoneStateManagerMock, movement: .up,
            settingsService: settingsServiceFake
        )
        sut.bpmChanged(bpm: getBpmSampleFromHeartZone(zone: zone1))
        sut.bpmChanged(bpm: getBpmSampleFromHeartZone(zone: zone1))
